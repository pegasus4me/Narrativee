    ...
    <div className="rounded-2xl border border-zinc-800/80 bg-zinc-950/40 p-5 transition-shadow flex flex-col gap-4 relative overflow-hidden text-zinc-200">
      ...
      {isEditing ? (
        <div className="flex flex-col gap-2">
          <textarea value={editText} onChange={(e) => onEditTextChange(e.target.value)} className="w-full min-h-[120px] rounded-xl border border-zinc-800 bg-zinc-900/50 p-3 text-xs leading-relaxed text-zinc-250 focus:outline-none focus:ring-1 focus:ring-[#e99ab1] font-normal" />
      ...
      {isRescheduling && (
        <div className="rounded-xl border border-[#e99ab1]/20 bg-[#e99ab1]/5 p-4 mt-2 grid gap-3 animate-in fade-in slide-in-from-top-2 duration-150">
          <div className="flex items-center gap-2 text-[10px] font-bold text-[#e99ab1]"><Calendar className="h-3.5 w-3.5" /> Reschedule Publishing Slot</div>
      ...
      {!isEditing && !isRescheduling && (
        <div className="flex items-center justify-between border-t border-zinc-800/80 pt-3 mt-1 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" disabled={isPublishing} onClick={onPublishNow} className="inline-flex items-center gap-1 rounded-lg bg-[#e99ab1] hover:bg-[#e99ab1]/90 text-white px-2.5 py-1 text-[10px] font-bold transition-colors disabled:opacity-50">
