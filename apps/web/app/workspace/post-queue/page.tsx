...
export default function PostQueuePage() {
  ...
  const groupedScheduledPosts = scheduledPosts.reduce((groups: any, post) => {
                          ...
                          >
                            <div className="flex items-center justify-between">
                              {isToday ? (
                                ...
                                <span className={`text-[10px] font-bold ${isCurrentMonth ? "text-zinc-700" : "text-zinc-400"}`}>
                                  {date.getDate()}
                                </span>
                              )}
                              {dayPosts.length > 0 && (
                                <span className="rounded-full bg-zinc-100 px-1.5 py-0.5 text-[8px] font-bold text-zinc-500 border border-zinc-200/50">
                                  {dayPosts.length}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="mt-2 flex-1 flex flex-col gap-1 overflow-hidden">
                              {dayPosts.slice(0, 3).map((post) => {
                                const platformColor =
                                  post.channel?.platform === "linkedin"
                                    ? "bg-blue-50 text-blue-750 border-blue-100"
                                    : post.channel?.platform === "x"
                                    ? "bg-zinc-900 text-zinc-100 border-zinc-800"
                                    : post.channel?.platform === "threads"
                                    ? "bg-zinc-50 text-zinc-900 border-zinc-200"
                                    : post.channel?.platform === "instagram"
                                    ? "bg-fuchsia-50 text-fuchsia-850 border-fuchsia-100"
                                ...
                                return (
                                  ...
                                  >
                                    <span className="shrink-0 text-[8px] font-semibold opacity-85">
                                      {formatFriendlyTime(post.scheduledAt)}
                                    </span>
                                    <span className="truncate opacity-90">
                                      {post.channel?.accountName || post.channel?.platform}
                                    </span>
                                  </div>
                                );
                              })}
                              {dayPosts.length > 3 && (
                                <div className="text-[8px] font-bold text-zinc-400 text-center mt-0.5">
                    ...
                    {Object.keys(groupedScheduledPosts).map((friendlyDate) => (
                      <div key={friendlyDate} className="flex flex-col gap-3">
                        ...
                        <div className="grid gap-4">
                          {groupedScheduledPosts[friendlyDate].map((post: any) => {
            ...
            <div className="flex items-start justify-between gap-4 pb-3.5 ">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <div className="flex items-center gap-1 rounded-full bg-zinc-50 border border-zinc-100 px-2.5 py-0.5 text-[10px] font-semibold text-zinc-600">
                  <Clock className="h-3 w-3 text-zinc-400" />
                  <span>{formatFriendlyTime(activeModalPost.scheduledAt)}</span>
                </div>
                <button
                  type="button"
                  onClick={() => {
            ...
            {editingPostId !== activeModalPost.id && reschedulingPostId !== activeModalPost.id && (
              <div className="flex items-center justify-between pt-4 mt-1 flex-wrap gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => startRescheduling(activeModalPost.id, activeModalPost.scheduledAt)}
                    className="inline-flex items-center gap-1 text-[11px] font-bold text-zinc-500 transition-colors"
                  >
