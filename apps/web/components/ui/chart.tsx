...
type ChartConfig = Record<string, { label?: string; color?: string; icon?: React.ComponentType }>
                            ...
                            {formatter && item?.value !== undefined && item.name ? (
                                ...
                                <>
                                    {itemConfig?.icon ? (
                                        ...
                                        <div className={cn("shrink-0 rounded-[2px] border-[--color-border] bg-[--color-bg]", indicator === "dot" && "h-2.5 w-2.5 rounded-[2px]", indicator === "line" && "w-1", indicator === "dashed" && "w-0 border-[1.5px] border-dashed bg-transparent")}
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
    ...
    if (key in config) configLabelKey = key
        ...
        const payloadKey = payloadPayload[key] as string
        if (payloadKey in config) configLabelKey = payloadKey
    }
